...
function Network() {
  ...
  return (
    <div className="w-screen h-[100vh] bg-bg-[#f0efe7] pt-[100px] px-[20px] flex flex-col items-center gap-[40px]">
      <Nav />
      ...
      {connections.length > 0 && (
        <div className="w-[100%] max-w-[900px] shadow-lg rounded-lg flex flex-col gap-[20px] min-h-[100px]">
          {connections.map((connection, index) => (
            <div key={connection._id} className="w-full min-h-[100px] p-[20px] flex justify-between items-center">
              <div>
                <div className="w-[60px] h-[60px] rounded-full overflow-hidden cursor-pointer">
                  <img src={connection.sender.profileImage || db} alt="" />
                </div>
              ...
              <div className="flex justify-center items-center gap-[10px]">
                <button className="text-[#2dc0ff] font-semibold" onClick={()=>handleAcceptConnection(connection._id)}>
                  <IoIosCheckmarkCircleOutline className="w-[36px] h-[36px] " />
                </button>

                <button className="text-[#ec4545] font-semibold" onClick={()=>handleRejectConnection(connection._id)}>
