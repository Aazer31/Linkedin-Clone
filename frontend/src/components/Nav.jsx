...
function Nav() {
  ...
  return (
    <div className='w-full h-[80px] bg-white fixed top-0 shadow-lg flex justify-between md:justify-around items-center px-[5px]'>
      ...
      {!activeSearch && <div><IoSearchSharp className='w-[23px] h-[23px] text-gray-700 
      lg:hidden' onClick={()=>setActiveSearch(true)}/></div>}

      <form className = {`w-[190px] lg:w-[350px] h-[40px] bg-[#f0efe7] lg:flex items-center gap-[10px] px-[10px] 
      ...
      <div className='flex justify-center items-center gap-[20px] relative'>
        ...
        {showPopup && <div className='w-[300px] min-h-[300px] bg-white shadow-lg absolute top-[75px] rounded-lg
        ...
        <div className='text-[18px] font-semibold text-gray-700'>
          ...
          <button className='w-[100%] h-[40px] rounded-full border-2
           border-[#2dc0ff] text-[#2dc0ff]'>View Profile</button>
          <div className='w-full h-[1px] bg-gray-700'></div>
          <div className='flex w-full justify-start items-center text-gray-600 gap-[10px] cursor-pointer'>
          <FaUserGroup className='w-[23px] h-[23px] text-gray-600'/>
          <div>My Networks</div>
        </div>
         <button className='w-[100%] h-[40px] rounded-full border-2
           border-[#ec4545] text-[#ec4545]' onClick={handleSignOut}>Sign Out</button>
        </div>}
